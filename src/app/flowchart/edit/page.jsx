...
export default function EditFlowcharts() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const degreeToFlowcharts = await Promise.all(degrees.map((degree) => getDegreeMapByDegree(degree.name)));
      const getCurrentEnvironment = await getFlowchartEnv();
  ...
  if (loading) {
    return <div className="flex align-center text-center justify-center items-center h-[100vh]">
